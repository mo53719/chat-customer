...
export const COLOR_DOWN = "#ff4d4f";
...
/** 趋势箭头样式 */
export function trendStyle(up: boolean) {
  return {
    fontSize: 12,
...
/** 计算环比变化百分比 */
export function calcTrend(curr: number, prev: number): { up: boolean; pct: number } {
  if (prev === 0) return { up: curr > 0, pct: curr > 0 ? 100 : 0 };
